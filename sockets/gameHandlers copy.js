...
const fetchQuestions = async (gameType, userId) => {
  ...
  if (gameType === 'solo' && userId) {
    ...
    if (excluded.length > 0) {
      // First attempt: fetch only unplayed questions
      const unplayedQuestions = await Question.aggregate([
  ...
  // For non-solo mode OR when solo pool is exhausted, select from entire database
